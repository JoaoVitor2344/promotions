...
exports.sendPromotionToTelegram = async (req, res) => {
  ...
  try {
    ...
    if (sent) {
      ...
      res
        .status(500)
        .json({ error: "Erro ao enviar mensagem para o Telegram." });
    }
  } catch (error) {
...
exports.autoApprovePromotions = async (req, res) => {
  try {
    const pendings = await prisma.promotion.findMany({
      where: { approved: false },
    });
    let approvedCount = 0;
    ...
    res.json({
      message: `Promoções aprovadas automaticamente: ${approvedCount}`,
    });
  } catch (error) {
    res
      .status(500)
      .json({ error: "Erro ao aprovar promoções automaticamente." });
  }
};
